...
class AirCargoProblem(Problem):
    ...
    def actions(self, state: str) -> list:
        ...
        for action in self.actions_list:
            ...
            for cond in action.precond_pos:
                if cond in fs.pos or cond not in fs.neg:
                    count +=1
            if count == len(action.precond_pos):
    ...
    def result(self, state: str, action: Action):
        ...
        for pos_state,neg_state in zip(fs.pos,fs.neg):
            if pos_state not in new_state.pos and pos_state not in new_state.neg:
                new_state.pos.append(pos_state)
            if neg_state not in new_state.neg and neg_state not in new_state.pos:
                new_state.neg.append(neg_state)
